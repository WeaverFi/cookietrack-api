...
const getETH = async (wallet: Address): Promise<NativeToken[]> => {
  try {
    let eth = new ethers.providers.JsonRpcProvider(rpcs[chain][0]);
    ...
    let eth = new ethers.providers.JsonRpcProvider(rpcs[chain][1]);
    let balance = parseInt(await eth.getBalance(wallet));
    if(balance > 0) {
      let newToken = await addNativeToken(chain, balance, wallet);
      return [newToken];
...
const getTokenBalances = async (wallet: Address): Promise<Token[]> => {
  let tokens: Token[] = [];
  let promises = eth_data.tokens.map((token: { address: Address, symbol: string }) => (async () => {
