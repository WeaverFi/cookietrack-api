...
const getBorrowedTokens = async (wallet: TerraAddress) => {
  const tokens: (DebtToken | NativeToken | Token)[] = [];

  // Borrowed tokens and Rewards
  const borrowerInfo = await query(market, {borrower_info: {borrower: wallet}});
  const borrowedBalance = parseInt(borrowerInfo.loan_amount);
  const borrowRewards = parseInt(borrowerInfo.pending_rewards);
  ...
  // Collateral Tokens
  const collateralBLuna = parseInt((await query(bLunaCustody, {borrower: {address: wallet}})).balance);
  const collateralBEth = parseInt((await query(bEthCustody, {borrower: {address: wallet}})).balance);
...
// Function to get ANC Governance Tokens:
const getAncGovTokens = async (wallet: TerraAddress) => {
  const govAncBalance = parseInt((await query(gov, {staker: {address: wallet}})).balance);
  if(govAncBalance > 0) {
    return [await addToken(project, 'staked', anc, 'ANC', 6, govAncBalance, wallet)];
  } else {
    return [];
  }
}
...
// Function to get staked ANC-UST LP Shares and Rewards:
const getStakedLP = async (wallet: TerraAddress) => {
  const tokens: (LPToken | Token)[] = [];
  const lpBalance = parseInt(await query(astroGenerator, {deposit: {lp_token: ancUstLPToken, user: wallet}}));
  const rewardsBalance = parseInt((await query(astroGenerator, {pending_token: {lp_token: ancUstLPToken, user: wallet}})).pending_on_proxy);
  if(lpBalance > 0) {
