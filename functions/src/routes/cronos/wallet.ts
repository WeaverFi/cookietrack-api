...
import { cronos_data } from '../../tokens';
...
export const get = async (req: Request) => {
  ...
  if(response.status === 'ok') {
    try {
      ...
      response.data.push(...(await getCRO(wallet)));
...
const getCRO = async (wallet: Address) => {
  let cro = new ethers.providers.JsonRpcProvider(rpcs[chain][0]);
  let balance = parseInt(await cro.getBalance(wallet));
  if(balance > 0) {
...
const getTokenBalances = async (wallet: Address) => {
  let tokens: Token[] = [];
  let promises = cronos_data.tokens.map(token => (async () => {
