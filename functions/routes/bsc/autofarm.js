...
const getVaultBalances = async (wallet) => {
  let balances = [];
  let autoRewards = 0;
  let poolLength = parseInt(await query(chain, registry, autofarm.bscRegistryABI, 'poolLength', []));
  let vaults = [...Array(poolLength).keys()];
  let promises = vaults.map(vaultID => (async () => {

    // AUTO Vault:
    if(vaultID === 0) {
      let balance = parseInt(await query(chain, autoVault, autofarm.bscRegistryABI, 'stakedWantTokens', [0, wallet]));
    ...
    // All Other Vaults:
      ...
      let balance = parseInt(await query(chain, registry, autofarm.bscRegistryABI, 'stakedWantTokens', [vaultID, wallet]));
      if(balance > 99) {
        let token = (await query(chain, registry, autofarm.bscRegistryABI, 'poolInfo', [vaultID]))[0];
        ...
        let rewards = parseInt(await query(chain, registry, autofarm.bscRegistryABI, 'pendingAUTO', [vaultID, wallet]));
        if(rewards > 0) {
          autoRewards += rewards;
        }
      }
