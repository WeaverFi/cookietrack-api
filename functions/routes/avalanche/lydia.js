...
const { lydia, minABI } = require('../../static/ABIs.js');
...
exports.get = async (req) => {
    ...
    if (wallet != undefined) {
        if (ethers.utils.isAddress(wallet)) {
            try {
                response.data.push(...(await GetAllFarmBalances(wallet)));
...
const GetAllFarmBalances = async (wallet) => {
    let balances = [];
    let farmCount = parseInt(await query(chain, registry, lydia.registryABI, 'poolLength', []));
    let farmList = [...Array(farmCount).keys()];
    let promises = farmList.map(farmId => (async () => {
...
const GetAutoLYDFarmBalance = async (wallet) => {
        ...
        if (shares > 0) {
            let exchangeRate = parseInt(await query(chain, autoLydFarm, lydia.lydFarmABI, 'getPricePerFullShare', []));
...
const GetMaximusFarmBalances = async (wallet) => {
        ...
        let promises = maximusFarms.map(farm => (async () => {
            let balance = parseInt(await query(chain, farm, lydia.maximusFarmABI, 'balanceOf', [wallet]));
            if (balance > 0) {
